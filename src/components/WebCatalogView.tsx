      ...
      <div className="p-8 bg-white border border-[#E6D5C3] rounded-3xl shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div>
          <span className="text-xs uppercase tracking-wider font-bold text-[#8B5E34]">
            Shareable Master Digital Lookbook
          </span>
          <h1 className="text-2xl font-bold font-serif text-[#3E2723] mt-1">
            KalaKriti Master Artisan Catalog
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {copied ? <Check className="w-4 h-4 text-[#8B5E34]" /> : <Share2 className="w-4 h-4" />} Share Catalog Link
          </button>
          <button
            type="button"
          ...
          >
            <div className="aspect-square rounded-2xl bg-[#FAF9F7] border border-[#E6D5C3] flex items-center justify-center p-3 overflow-hidden">
              {product.imageEnhanced ? (
                <img src={product.imageEnhanced} alt={product.name} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
              ) : (
            ...
            <div className="space-y-1 text-xs">
              ...
              <p className="text-[11px] text-[#8C7355] line-clamp-2">{product.descriptionEnglish}</p>
              <div className="flex items-center justify-between pt-2 border-t border-[#E6D5C3]">
                <span className="font-serif font-bold text-sm text-[#8B5E34]">₹{product.price.toLocaleString('en-IN')}</span>
